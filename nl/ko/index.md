...
# {{site.data.keyword.Bluemix_local_notm}} 및 {{site.data.keyword.Bluemix_dedicated_notm}} 관리
{: #mng}

{{site.data.keyword.Bluemix_local_notm}} 또는 {{site.data.keyword.Bluemix_dedicated_notm}}에 대한 관리자 액세스 권한이 있는 경우에는 **관리** 페이지로 이동하여 리소스 관리, 할당 사용량에 대한 모니터링, 사용자 권한 관리, 업그레이드 알림에 대한 스케줄링, 보안 보고서와 로그 보기 등의 작업을 수행할 수 있습니다. 영역을 작성하고 [사용자 역할 및 권한](/docs/admin/index.html#oc_useradmin)을 설정하여 조직을 관리할 수 있습니다. [조직 관리](/docs/admin/orgs_spaces.html)를 참조하십시오.
{:shortdesc}
...
|EU 시스템 액세스 관리| **관리 &gt; EU 시스템 액세스**를 클릭하여 EU 시스템 액세스 정책을 관리하고 액세스 요청을 보십시오. [EU의 클라우드 시스템에 대한 액세스 관리](/docs/admin/index.html#oc_euaccess)를 참조하십시오. |
{: caption="표 1. {{site.data.keyword.Bluemix_notm}} 로컬 또는 데디케이티드 인스턴스 관리를 위한 관리 태스크" caption-side="top"}

**참고**: {{site.data.keyword.Bluemix_notm}} 콘솔의 EU 시스템 액세스 옵션은 사용자 환경이 EU 관리 클라우드가 되도록 구성된 경우에만 사용 가능합니다.
...
| 유형| **이메일**을 선택하십시오.|
| 이벤트| **임계값**을 선택하십시오.|
| 임계값| 평균 CPU, 조직 할당량, 실제 디스크, 실제 메모리, 예약 디스크 또는 예약 메모리에 대해 알림을 받을 임계값의 유형을 선택하십시오.|
...
| **IBM 값** | **설명** | **이벤트 유형**|
|----------------|----------------|------------------------|
| {{content.average_cpu}} | 평균 CPU 임계값 | 임계값|
...
블랙아웃 기간은 IBM에서 승인해야 하며, 승인을 받는 데 걸리는 시간은 경우에 따라 다릅니다. 블랙아웃이 제출된 후 7일 내에 발생하도록 이미 스케줄된 배치와 겹치는 블랙아웃 기간은 승인에 더 오래 걸릴 수 있습니다. 요청한 블랙아웃이 승인되면 IBM에서 블랙아웃 기간 동안 현재 스케줄된 기존 업데이트를 모두 취소합니다. IBM에서는 이러한 업데이트에 대한 새 레코드를 작성한 다음 승인된 블랙아웃 날짜를 제외한 날짜로 스케줄링합니다.
...
## EU의 클라우드 시스템에 대한 액세스 관리
{: #oc_euaccess}

{{site.data.keyword.IBM_notm}} 담당자는 정기적으로 클라우드 시스템에 액세스하여 인시던트에 대한 응답으로 유지보수 활동을 수행하거나 유지보수 업데이트를 배치합니다. {{site.data.keyword.IBM_notm}}에서는 EU에 있는 {{site.data.keyword.Bluemix_dedicated_notm}} 시스템에서 유지보수 활동을 수행할 수 있도록 EU에서 담당자를 지정하기 위해 모든 노력을 다하고 있습니다. 지정할 수 없는 경우 {{site.data.keyword.IBM_notm}}에서는 EU 클라우드 시스템의 유지보수를 수행할 담당자를 EU 외부에서 지정할 수 있습니다. EU 외부의 {{site.data.keyword.IBM_notm}} 담당자가 EU 클라우드 시스템에 액세스하려면 사용자가 필요한 승인 레벨을 제어할 수 있어야 합니다. 액세스가 액세스 제어 정책에 따라 승인되면 {{site.data.keyword.IBM_notm}}에서는 임시 신임 정보를 지정된 담당자에게 제공합니다.

관리 콘솔에서 사용 가능한 정책 옵션을 통해 액세스를 제어합니다. **관리 &gt; EU 시스템 액세스**를 클릭하십시오. 정책은 EU 외부의 {{site.data.keyword.IBM_notm}} 담당자가 EU 클라우드 시스템에 액세스할 수 있도록 고객이 요구하는 승인 레벨을 판별합니다. 비EU 액세스 정책을 설정하고, 구성된 정책에서 생성된 액세스 요청을 승인하거나 거부하고, 승인 요청의 히스토리를 찾아볼 수 있습니다. _EU 시스템 액세스_ 보기에는 세 개의 탭이 있습니다.
 
* 정책
* 보류 중인 요청
* 요청 히스토리

**참고:** {{site.data.keyword.Bluemix_notm}} 콘솔의 EU 시스템 액세스 옵션은 사용자 환경이 EU 관리 클라우드 시스템이 되도록 구성된 경우에만 사용 가능합니다.

### 액세스 정책 설정 

EU 시스템 액세스 정책을 관리하려면 **관리 &gt; EU 시스템 액세스 &gt; 정책**을 클릭하십시오.

**정책** 섹션에서 시스템에 액세스하기 위해 EU 외부에 있는 IBM 담당자에게 필요한 승인 레벨을 지정할 수 있습니다. 비EU 담당자의 EU 클라우드 시스템에 대한 액세스는 IBM 승인 전에 필요합니다. 세 가지 사용 가능한 정책 옵션은 IBM 승인을 포함해 고객 승인이 필요한지 여부와 시기를 판별합니다.

<ul>
<li>고객 승인이 필요하지 않습니다. EU 외부의 IBM에서 승인한 담당자는 사용자 승인 없이 클라우드 시스템에 액세스할 수 있습니다.</li>
<li>고객 승인이 항상 필요합니다. EU 외부의 IBM 담당자는 명시적 승인을 통해서만 클라우드 시스템에 액세스할 수 있습니다.</li>
<li>사용자 정의 정책입니다. 사용자 승인은 필요한 액세스에 대한 유지보수 활동의 유형과 조건에 따라 필요합니다.</li>
</ul>

사용자 정책이 고객 승인을 위해 호출되면, 액세스 요청이 자동으로 승인되는 _자동 승인_ 기간을 선택적으로 정의할 수 있습니다.

다음은 사용자 정의 정책에 대한 다양한 유형의 활동 및 이와 연관된 조건의 목록과 설명입니다.

{: #ld_table18}

| **활동** | **조건** | **설명** |       
|-----------------|-------------------|-------------------|
| 인시던트| 블로커 | 지정된 컴포넌트가 액세스되지 않거나 예상대로 작동되지 않습니다. 환경은 크게 영향을 받습니다. 임시 해결책은 없습니다. |
| 인시던트| 비블로커 | 지정된 컴포넌트에 문제가 가끔씩 발생하거나 컴포넌트의 일부가 예상대로 작동되지 않습니다. 환경이 크게 영향을 받지 않거나 임시 해결책이 있습니다. |
| 유지보수 업데이트| 비상 배치 | 업데이트는 인시던트를 해결하거나 방지하는 데 필요합니다. |  
| 유지보수 업데이트| 일반 배치 | 업데이트는 기능성, 사용성, 신뢰성 또는 성능을 개선하기 위한 서비스 또는 컴포넌트의 일반 업그레이드의 일부입니다. |
{: caption="표 18. EU 사용자 정의 정책" caption-side="top"}

### 보류 중인 요청 처리 

주의가 필요한 액세스 요청을 보려면 **관리 &gt; EU 시스템 액세스 &gt; 보류 중인 요청**을 클릭하십시오.

**보류 중인 요청** 테이블에서 EU 외부에 있는 IBM 담당자의 EU 시스템 액세스 요청을 승인하거나 거부할 수 있습니다. 보류 중인 요청에 대한 일부 정보는 테이블 열로 표시됩니다. 지정된 요청에 대한 테이블 행을 펼치면 추가 세부사항이 제공됩니다.  

하나 이상의 요청을 승인하거나 거부하려면 요청의 왼쪽에 있는 하나 이상의 선택란을 선택한 다음 **승인** 또는 **거부** 단추를 선택하십시오. 자동 승인 정책을 통해 자동으로 또는 고객이 수동으로 수행할 수 있는 액세스 요청이 승인되거나 거부되면, 해당 레코드가 **보류 중인 요청** 탭에서 **요청 히스토리** 탭으로 이동됩니다. 액세스 요청을 승인하면 시스템에 액세스하는 다양한 IBM 담당자의 레코드를 볼 수 있습니다. 

**참고**: 액세스 요청은 특정 유지보수 활동과 연관됩니다. 이 유지보수 활동은 여러 IBM 담당자가 수행할 수 있습니다. 액세스 요청을 승인하면 여러 IBM 담당자의 시스템 액세스를 잠재적으로 승인한 것입니다. 클라우드 시스템에 액세스하는 각 개인은 여전히 IBM의 개별 승인과 신임 정보를 필요로 하지만 업데이트 또는 인시던트와 연관된 모든 필수 활동에 대해서는 한 번만 승인하면 됩니다. 

각 승인 요청에 대한 요약 정보는 테이블 행에 제공됩니다. 테이블 행을 펼치면 추가 정보가 표시됩니다.

<dl>
<dt>상태</dt>
<dd>액세스 요청의 상태입니다(보류 중, 승인됨 또는 거부됨).</dd>
<dt>활동 유형</dt>
<dd>필요한 액세스에 대한 활동 유형입니다(유지보수 업데이트 또는 인시던트).</dd>
<dt>레코드 ID</dt>
<dd>필요한 액세스에 대한 활동을 추적하는 데 사용되는 레코드의 ID입니다. 값은 활동이 유지보수 업데이트인 경우 실제 레코드에 링크됩니다.</dd>
<dt>요청 날짜</dt>
<dd>액세스 승인이 고객으로부터 요청된 날짜 및 시간입니다.</dd>
<dt>자동 승인 시간</dt>
<dd>요청이 자동으로 승인되기 전까지 남은 시간입니다. 자동 승인이 액세스 정책의 일부로 지정된 경우에만 해당됩니다. 자동 승인 기간을 설정한 경우 지정된 담당자가 IBM에서 승인될 때 자동 승인에 대한 카운트다운이 시작됩니다. 자동 승인이 액세스 정책의 일부로 지정된 경우에도 유지보수 업데이트에 대한 승인 요청에 자동 승인 시간이 포함되지 않을 수 있습니다. 사전 승인 요청인 경우 세부사항에 향후 특정 시간에 대한 요청임을 표시하는 참고사항이 포함됩니다.</dd>
<dt>설명</dt>
<dd>클라우드 시스템에 대한 액세스 요청의 이유입니다.</dd>
<dt>심각도(Severity)</dt>
<dd>인시던트의 심각도를 표시합니다. 인시던트에 대해서만 표시됩니다.</dd>
<dt>비상 배치 </dt>
<dd>유지보수 업데이트에 비상 배치가 필요한지 여부를 표시합니다. 유지보수 업데이트에 대해서만 표시됩니다.</dd>
<dt>참고</dt>
<dd>요청이 향후에 배치하도록 스케줄된 유지보수 업데이트의 일부인 사전 승인을 위한 요청인 경우에 표시합니다. 이와 같은 요청을 승인할 때 유지보수 업데이트가 배치를 위해 스케줄된 경우 클라우드 시스템에 대한 액세스를 사전 승인합니다. 요청을 사전 승인하면 시스템이 실제로 액세스될 때 추가 승인이 필요하지 않습니다.</dd>
</dl>

**참고**: 알림 구독은 새 액세스 요청이 작성되었을 때 이메일을 발송하거나 Webhook을 호출하도록 구성될 수 있습니다. 자세한 정보는 **알림 및 알림 구독**의 절을 참조하십시오(/docs/hybrid/index.html#oc_eventsubscription).

### 요청 히스토리 보기 

시스템에 대한 이전 액세스 요청을 보려면 **관리 &gt; EU 시스템 액세스 &gt; 요청 히스토리**를 클릭하십시오.

**요청 히스토리** 섹션에서 EU의 외부에 있는 IBM 담당자의 이전 액세스 요청을 모두 볼 수 있습니다.

각 히스토리 요청에 대한 요약 정보는 테이블 행에 제공됩니다. 테이블 행을 펼치면 추가 정보가 표시됩니다.

<dl>
<dt>상태</dt>
<dd>액세스 요청 상태입니다(승인됨 또는 거부됨).</dd>
<dt>활동 유형</dt>
<dd>필요한 액세스에 대한 활동 유형입니다(유지보수 업데이트 또는 인시던트).</dd>
<dt>레코드 ID</dt>
<dd>업데이트 또는 인시던트 활동을 추적하는 데 사용되는 레코드의 ID입니다. 값은 활동이 유지보수 업데이트인 경우 실제 레코드에 링크됩니다.</dd>
<dt>요청 날짜</dt>
<dd>액세스 승인이 고객으로부터 요청된 날짜 및 시간입니다.</dd>
<dt>조치 날짜</dt>
<dd>고객이 요청을 승인하거나 거부한 날짜 및 시간입니다. EU 액세스 정책이 IBM 승인을 위해 구성된 경우에만 필드에 __자동 승인__이 표시됩니다.</dd>
<dt>설명</dt>
<dd>클라우드 시스템에 대한 액세스 요청의 이유입니다.</dd>
<dt>심각도(Severity)</dt>
<dd>인시던트의 심각도를 표시합니다. 인시던트에 대해서만 표시됩니다.</dd>
...
<dd>유지보수 업데이트에 비상 배치가 필요한지 여부를 표시합니다. 유지보수 업데이트에 대해서만 표시됩니다.</dd>
<dt>참고</dt>
<dd>요청의 추가 관련 조건을 설명합니다. 예를 들어, 이 필드는 요청이 향후에 배치하도록 스케줄된 유지보수 업데이트의 일부인 사전 승인을 위한 요청인 경우에 표시하는 데 사용됩니다.</dd>
<dt>액세스 히스토리</dt>
<dd>시스템에 액세스하는 모든 사용자에 대해 IBM이 액세스를 승인한 날짜 및 시간과 해당 사용자의 액세스 권한이 부여되고 취소된 날짜 및 시간을 표시합니다. 예를 들어, 개인이 IBM 담당자 1로 식별됩니다.</dd>
</dl>

**참고**: 새 액세스 요청이 **승인됨** 상태로 자동으로 전이되고 액세스 정책이 IBM 승인만 요구하도록 구성된 경우 **요청 히스토리** 섹션에 표시됩니다.
