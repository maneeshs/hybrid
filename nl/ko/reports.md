...
<dd>실행된 명령입니다.
<ul>
<li>Create Window [id] - 유지보수 업데이트를 위한 창을 작성합니다. 업데이트 기간 로그는 사용자가 환경에 대한 업데이트 기간을 관리(작성)할 때 생성됩니다. 업데이트 기간은 블랙아웃이며 배치 작업에 유용한 기간입니다. 이는 고객이 배치를 승인하고 스케줄링할 필요 없이 시스템을 중단하지 않는 배치를 허용하거나 허용하지 않는 경우입니다.
<li>Update Window [id] - 유지보수 업데이트 기간을 업데이트합니다. 이 이벤트는 사용자가 환경에 대한 유지보수 업데이트 기간을 업데이트할 때 생성됩니다.
<li>SECURITY_MGMT_CONFIG - 업데이트 기간을 삭제합니다. 이 이벤트는 사용자가 환경에 대한 업데이트 기간을 위임할 때 생성됩니다.
