...
<dd>事件发生的日期。</dd>
<dt>事件名称</dt>
<dd>事件的类型（用户无法通过 SSH 登录 - 密码不正确、认证失败、会话关闭、针对用户接受密码、为用户打开会话）。
</dd>
<dt>低级别类别</dt>
<dd>事件详细信息（SSH 登录失败、会话关闭、主机登录成功、会话已打开）。
</dd>
<dt>用户名</dt> 
...
报告因现有的防火墙规则，访问请求被拒绝时，Vyatta 防火墙设备生成的事件。
...
报告管理员在 Bluemix 系统上启动 SSH 会话时，操作系统生成的事件。
...
<dt>用户</dt> 
<dd>执行操作的用户的用户名。以下用户是向数据库定义的用户。
<ul>
<li>UAADB：vcap、root、backup_user。
<li>CCDB：vcap、ccadmin、backup_user。
<li>以上用户之外的使用需要调查何人执行的操作。使用“操作系统管理安全报告”和“操作系统登录安全报告”可以完成此操作。
</ul>
</dd>
<dt>源</dt> 
<dd>用户启动命令的 IP 地址。
...
<dt>DB_Statement</dt>
<dd>操作的类型（所有数据定义语句（如 CREATE、ALTER 和 DROP 语句），外加数据修改语句（如 INSERT、UPDATE、DELETE、TRUNCATE和 COPY FROM）</dd>
<dt>DB_Name</dt>
<dd>执行操作的数据库名称</dd>
</dl>
...
<li>向用户 [用户标识] 添加许可权 [许可权]：向指定用户添加许可权。许可权可以包括：
ops.admin、ops.login、ops.reports.read、ops.reports.write、ops.catalog.read、ops.catalog.write、ops.users.read 和 ops.user.write。
<li>从用户 [用户标识] 除去许可权 [许可权]：除去许可权 - 从指定用户除去许可权。许可权可以包括：ops.admin、ops.login、ops.reports.read、ops.reports.write、ops.catalog.read、ops.catalog.write、ops.users.read 和 ops.users.write。
<li>创建组织 [名称]：创建具有指定名称的组织。
...
<dd>注册模板 [标识]。使具有指定标识的样板或运行时在目录中为所有用户显示为选项。</dd>
<dd>取消注册模板 [标识]。使样板或运行时不再在目录中为所有用户显示为选项。</dd>
<dd>将服务套餐 [标识] 更新为公共 = 具有可视性的 [ 公共 ] = [ 标识 ]。更新指定服务套餐的可视性。如果公共设置为 true，那么所有组织都可看到套餐。如果公共设置为 false，那么可能会提供一组组织标识，以指出哪些组织可以看到此服务套餐。</dd>
<dd>添加服务代理程序 [名称]。添加具有指定名称的服务代理程序。</dd>
<dd>删除服务代理程序 [名称]。删除具有指定名称的服务代理程序。</dd>
<dd>更新 buildpack [标识]。针对具有指定标识的构建包更新 buildpack 订购。</dd>
<dt>日志源</dt>
<dd>事件的源（生成事件的服务器的 IP 地址/主机名）</dd>
</dl>
...
<dd>事件的类型 (SECURITY_RUNTIME)。
<ul>
<li>SECURITY_DATA_SYNC：将报告 [名称] 上载到类别 [名称] 日期 [日期]。
删除标识为 [标识] 的报告
...
<dd>命令已执行。
<ul>
<li>将报告 [名称] 上载到类别 [名称] 日期 [日期]：上载 - 将具有指定文件名和日期的报告上载到指定的类别。
...
<dd>命令已执行。
<ul>
<li>创建窗口[id]: Creates "创建"环境的更新窗口。更新窗口是用于部署的中断和首选窗口。当客户在无需核准和安排非中断性部署的情况下，允许/不允许进行此种部署时生成此事件。
<li>更新窗口 [标识]：更新维护更新窗口。当用户更新环境的维护更新窗口时生成此事件。
<li>SECURITY_MGMT_CONFIG：删除更新窗口。当用户委派环境的更新窗口时生成此事件。
<li>创建预订 [标识]：创建事件预订。当用户为更新事件或常事件创建预订时生成事件预订审计日志。通过此事件，当他们的系统上发生某些状况时，他们可以获得通知。
<li>更新预订 [标识]：更新事件预订。当用户为更新事件或常事件更新预订时生成此事件。
<li>删除预订 [标识]：删除事件预订。当用户为更新事件或常事件删除预订时生成此事件。
<li>创建主题回复 [标识]：回复主题。当客户核准并安排中断性更新时生成主题回复审计日志。
</ul>
</dd>
<dt>日志源</dt>
<dd>事件的源（生成事件的服务器的 IP 地址/主机名）。</dd>
...
<li>UserCreatedEvent ('["user_id=ca3a811f-1778-4103-9553-537788ed4c4e","username=equaranta"]’)。使用 (username=equaranta 和 id = ca3a811f-1778-4103-9553-537788ed4c4e) 创建用户。
<li>GroupModifiedEvent ('{" "group_name":"ops.reports.read","members":["0625ff9a- 8a59-4cca-a80a-8e3b51f3dd21","5005f0f8-e090-4cabb51d- 2ceee70acf0e"}')。修改名称为 ops.reports.read 的组，产生的成员资格 (id 0625ff9a- 8a59-4ccaa80a- 8e3b51f3dd21","5005f0f8- e090-4cab-b51d- 2ceee70acf0e)。
<li>UserDeletedEvent ('["UserDeletedEvent ('["user_id=5cd5f412-4bbb-4c44-b44b- 713a5bc6144d","username=818811853"]'):"]'。删除标识值 =5cd5f412-4bbb- 4c44-b44b-713a5bc6144 的用户。
</ul>
</dd>
<dt>消息</dt>
...
报告 Bluemix 生成的事件，这些事件可能帮助诊断故障（例如，Bosh 代理程序无法部署或服务未启动）。
...
<dt>uid</dt>
<dd>目标用户标识，命令运行时使用该标识（例如，在运行命令之前，auid=55044 登录并执行“sudo su”(root) -> uid=0）。
<dt>exe_key</dt>
<dd>执行的命令。</dd>
<dt>源 IP</dt>
<dd>用户启动命令的 IP 地址。
