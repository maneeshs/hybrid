...
# 管理 {{site.data.keyword.Bluemix_local_notm}} 和 {{site.data.keyword.Bluemix_dedicated_notm}}
{: #mng}

如果您具有 {{site.data.keyword.Bluemix_local_notm}} 或 {{site.data.keyword.Bluemix_dedicated_notm}} 的管理员访问权，请转至**管理**页面来管理资源、监视配额使用情况、管理用户许可权、安排升级通知，以及查看安全报告和日志等。您可以通过创建空间并设置[用户角色和许可权](/docs/admin/index.html#oc_useradmin)来管理组织；请参阅[管理组织](/docs/admin/orgs_spaces.html)。
{:shortdesc}
...
|管理欧盟系统访问| 单击**管理 &gt; 欧盟系统访问**以管理“欧盟系统访问”策略以及查看访问请求。请参阅[管理对欧盟中云系统的访问](/docs/admin/index.html#oc_euaccess)。|
{: caption="表 1. 用于管理 {{site.data.keyword.Bluemix_notm}} Local 或 Dedicated 实例的管理任务" caption-side="top"}

**注**：仅当环境配置为欧盟管理的云时，{{site.data.keyword.Bluemix_notm}} 控制台中的“欧盟系统访问”选项才可用。
...
| 类型| 选择**电子邮件**。|
| 事件| 选择**阈值**。|
| 阈值| 选择要获取相关通知的阈值的类型：平均 CPU、组织配额、物理磁盘、物理内存、保留磁盘或保留内存。|
...
| **IBM 值**| **描述**| **事件类型** |
|----------------|----------------|------------------------|
| {{content.average_cpu}} | 平均 CPU 阈值| 阈值|
...
中断时段需要 IBM 批准，多长时间能获得批准不一定。如果中断时段与已经安排为从提交中断时起 7 天内执行的部署重叠，那么可能需要更长时间才能批准。IBM 在批准了请求的中断之后，会取消当前安排在中断时段中的所有现有更新。IBM 还会为这些更新创建新记录，并将其安排在批准的中断日期以外的时间。
...
配额表示在环境中创建组织时为其分配的资源限制。组织内空间中的任何应用程序或服务都会使用一定的已分配配额。要管理组织的配额，请完成以下步骤：
...
{{site.data.keyword.IBM_notm}} 人员会经常访问云系统以执行维护活动，即响应事件或部署维护更新。{{site.data.keyword.IBM_notm}} 会尽量指定欧盟 (EU) 人员对位于 EU 的 {{site.data.keyword.Bluemix_dedicated_notm}} 系统执行维护活动。若无法指定此类人员，那么 {{site.data.keyword.IBM_notm}} 可能会指定 EU 以外的人员对 EU 云系统执行维护。您可控制在 EU 以外的 {{site.data.keyword.IBM_notm}} 人员访问 EU 云系统之前所需的批准级别。根据您的访问控制策略批准访问后，{{site.data.keyword.IBM_notm}} 会向指定的人员提供临时凭证。

您可以通过管理控制台中可用的策略选项来控制访问。单击**管理 &gt; 欧盟系统访问**。这些策略将确定 EU 以外的 {{site.data.keyword.IBM_notm}} 人员访问 EU 云系统之前客户所需的批准级别。您可以设置非欧盟访问策略，批准或拒绝从配置的策略生成的访问请求，以及浏览批准请求的历史记录。_欧盟系统访问_视图有三个选项卡：
 
* 策略
* 暂挂请求
* 请求历史记录

**注**：仅当环境配置为 EU 管理的云系统时，{{site.data.keyword.Bluemix_notm}} 控制台中的“欧盟系统访问”选项才可用。

### 设置访问策略 

要管理“欧盟系统访问”策略，请单击**管理 &gt; 欧盟系统访问 &gt; 策略**。

在**策略**部分中，可以指定位于 EU 以外的 IBM 人员访问您系统所需的批准级别。非 EU 人员对 EU 云系统的任何访问都需要事先获得 IBM 批准。有三个可用策略选项用于确定是否以及何时需要客户批准，包括 IBM 批准。

<ul>
<li>无需客户批准。IBM 批准的欧盟以外人员可以访问云系统，而无需您批准。</li>
<li>始终需要客户批准。EU 以外的 IBM 人员仅当获得明确批准时，才能访问云系统。</li>
<li>定制策略。根据需要访问权的维护活动的类型和条件，确定是否需要您批准。</li>
</ul>

策略要求客户批准时，可以选择定义会自动批准访问请求的_自动批准_时间段。

对于定制策略，下面列出并描述了各种类型的活动及其关联条件：

{: #ld_table18}

| **活动**| **条件** | **描述**|       
|-----------------|-------------------|-------------------|
| 事件| 阻止程序| 指定的组件不可访问或未按预期运行。环境受到严重影响。没有变通方法。|
| 事件| 非阻止程序| 指定的组件存在间歇性问题，或该组件的一部分未按预期工作。环境并未受到严重影响，或者有变通方法。|
| 维护更新| 紧急部署| 需要进行更新以解决事件或避免事件发生。|  
| 维护更新| 正常部署| 此更新是服务或组件常规升级的一部分，用于增强其功能、易用性、可靠性或性能。|
{: caption="表 18. EU 定制策略" caption-side="top"}

### 解决暂挂请求 

要查看需要您关注的访问请求，请单击**管理 &gt; 欧盟系统访问 &gt; 暂挂请求**。

在**暂挂请求**表中，可以批准或拒绝位于 EU 以外的 IBM 人员访问 EU 系统的请求。有关暂挂请求的一些信息会显示为表列。展开给定请求的表行时，会提供其他详细信息。  

要批准或拒绝一个或多个请求，请选择请求左侧的一个或多个复选框，然后选择**批准**或**拒绝**按钮。批准或拒绝访问请求（可以由客户手动完成或通过自动批准策略自动完成）后，其记录会从**暂挂请求**选项卡移至**请求历史记录**选项卡。批准访问请求后，您将能够查看不同 IBM 人员访问系统的记录。 

**注**：访问请求与特定维护活动相关联。该维护活动可能由多名 IBM 人员执行。批准访问请求时，您有可能会批准多名 IBM 人员对该系统进行访问。访问云系统的每个人员仍需要 IBM 单独批准并提供凭证，但您只需对与该更新或事件关联的所有必需活动授予一次批准即可。 

每个批准请求的摘要信息在表行中提供。展开表行时，将显示其他信息：

<dl>
<dt>状态</dt>
<dd>访问请求的状态：“暂挂”、“已批准”或“已拒绝”。</dd>
<dt>活动类型</dt>
<dd>需要访问权的活动的类型：维护更新或事件。</dd>
<dt>记录标识</dt>
<dd>用于跟踪需要访问权的活动的记录的标识。如果活动是维护更新，那么此值会链接到实际记录。</dd>
<dt>请求日期</dt>
<dd>客户请求访问批准的日期和时间。</dd>
<dt>距离自动批准的时间</dt>
<dd>距离自动批准请求的剩余时间。仅当在访问策略中指定了自动批准时才适用。请注意，如果设置了自动批准时间段，那么在 IBM 批准指定的人员时，该自动批准时间段将开始倒计时。对于维护更新的批准请求，即便在访问策略中指定了“自动批准”，也可能没有“距离自动批准的时间”。如果这是预批准请求，那么详细信息将包含注释，指示该请求用于未来特定时间。</dd>
<dt>描述</dt>
<dd>请求访问云系统的理由。</dd>
<dt>严重性</dt>
<dd>指示事件的严重性。此项仅对事件显示。</dd>
<dt>紧急部署</dt>
<dd>指示维护更新是否需要紧急部署。此项仅对维护更新显示。</dd>
<dt>注释</dt>
<dd>指示这是作为安排在未来部署的维护更新一部分的预批准请求。如果批准了此类请求，那么在安排维护更新进行部署时，将预批准对云系统的访问权。预批准请求后，在实际访问系统时无需进一步批准。</dd>
</dl>

**注**：通知预订可以配置为在创建了新的访问请求时，向您发送电子邮件或调用 Webhook。有关更多信息，请参阅**通知和通知预订**部分 (/docs/hybrid/index.html#oc_eventsubscription)。

### 查看请求历史记录 

要查看系统的先前访问请求，请单击**管理 &gt; 欧盟系统访问 &gt; 请求历史记录**

在**请求历史记录**部分中，可以查看位于 EU 以外的 IBM 人员的所有先前访问请求。

每个历史请求的摘要信息在表行中提供。展开表行时，将显示其他信息：

<dl>
<dt>状态</dt>
<dd>访问请求的状态：“已核准”或“已拒绝”。</dd>
<dt>活动类型</dt>
<dd>需要访问权的活动的类型：维护更新或事件。</dd>
<dt>记录标识</dt>
<dd>用于跟踪更新或事件活动的记录的标识。如果活动是维护更新，那么此值会链接到实际记录。</dd>
<dt>请求日期</dt>
<dd>客户请求访问批准的日期和时间。</dd>
<dt>操作日期</dt>
<dd>客户批准或拒绝请求的日期和时间。如果将“欧盟访问”策略配置为仅需要 IBM 批准，那么此字段会显示__自动批准__。</dd>
<dt>描述</dt>
<dd>请求访问云系统的理由。</dd>
<dt>严重性</dt>
<dd>指示事件的严重性。此项仅对事件显示。</dd>
...
<dd>指示维护更新是否需要紧急部署。此项仅对维护更新显示。</dd>
<dt>注释</dt>
<dd>描述有关请求的任何其他相关条件。例如，此字段将用于指示这是作为安排在未来部署的维护更新一部分的预批准请求。</dd>
<dt>访问历史记录</dt>
<dd>对于每个访问系统的人员，此项会显示 IBM 批准访问的日期和时间，以及授予和撤销该人员访问权的日期和时间。个人会标识为 IBM Personnel 1，等等。</dd>
</dl>

**注**：如果已将访问策略配置为仅需要 IBM 批准，那么新的访问请求会自动转换为**已批准**状态并显示在**请求历史记录**部分中。
