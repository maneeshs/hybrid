...
|EU システム・アクセスの管理 | EU システム・アクセス・ポリシーを管理したり、アクセス要求を表示したりするには、**「管理」&gt;「EU システム・アクセス」**をクリックします。[EU 内のクラウド・システムへのアクセスの管理](/docs/admin/index.html#oc_euaccess)を参照してください。|
{: caption="表 1. {{site.data.keyword.Bluemix_notm}} Local または Dedicated のインスタンスを管理するための管理用タスク" caption-side="top"}

**注**: {{site.data.keyword.Bluemix_notm}} コンソールで EU システム・アクセスのオプションを使用できるのは、ご使用の環境が EU 管理対象クラウドになるように構成されている場合に限定されます。
...
| タイプ | **「E メール」**を選択します。 |
| イベント | **「しきい値」**を選択します。 |
| しきい値 | 通知するしきい値のタイプ (「平均 CPU (Average CPU)」、「組織割り当て量 (Organization Quota)」、「物理ディスク (Physical Disk)」、「物理メモリー (Physical Memory)」、「予約済みディスク (Reserved Disk)」、または「予約済みメモリー (Reserved Memory)」) を選択します。 |
...
| **IBM 値** | **説明** | **イベント・タイプ** |
|----------------|----------------|------------------------|
| {{content.average_cpu}} | 平均 CPU しきい値 | しきい値 |
...
ブラックアウト・ウィンドウは IBM の承認が必要であり、承認を得るために要する時間は変わることがあります。 ブラックアウトの処理依頼を送信した時点から 7 日以内の実施が既にスケジュールされているデプロイメントとオーバーラップするブラックアウト期間は、承認までの時間が長くなる場合があります。要求したブラックアウトが承認された場合、IBM はその時点でブラックアウト期間中にスケジュールされている既存の更新をキャンセルします。さらに IBM は、これらの更新用に新たなレコードを作成し、承認されたブラックアウト日以外の期間に更新をスケジュールします。
...
## EU 内のクラウド・システムへのアクセスの管理
{: #oc_euaccess}

{{site.data.keyword.IBM_notm}} 担当者は、インシデントに対応するため、または保守更新をデプロイするために定期的にクラウド・システムにアクセスして保守アクティビティーを実行します。{{site.data.keyword.IBM_notm}} では、EU 内に配置されている {{site.data.keyword.Bluemix_dedicated_notm}} システムの保守アクティビティーを実行するにあたって、できる限り EU の担当者を割り当てるようにしています。それが可能でない場合、{{site.data.keyword.IBM_notm}} は EU 外部の担当者を割り当てて、EU クラウド・システムの保守を実行することがあります。EU 外部の {{site.data.keyword.IBM_notm}} 担当者がお客様の EU クラウド・システムにアクセスするために必要な承認レベルをお客様が制御できます。アクセス制御ポリシーに従ってアクセスが承認されると、割り当てられた担当者には、{{site.data.keyword.IBM_notm}} から一時的な資格情報が提供されます。

アクセスの制御は、管理コンソールで使用可能なポリシー・オプションから行います。**「管理」&gt;「EU システム・アクセス」**をクリックします。 ポリシーによって、お客様が必要とする、EU 外部の {{site.data.keyword.IBM_notm}} 担当者が EU クラウド・システムにアクセスするための承認レベルが決まります。非 EU アクセス・ポリシーをセットアップし、構成されているポリシーに基づいてアクセス要求を承認または拒否し、承認要求の履歴を表示できます。_「EU システム・アクセス」_ビューには、以下の 3 つのタブがあります。
 
* ポリシー
* 未処理要求
* 要求履歴

**注:** {{site.data.keyword.Bluemix_notm}} コンソールで EU システム・アクセスのオプションを使用できるのは、ご使用の環境が EU 管理対象クラウド・システムになるように構成されている場合に限定されます。

### アクセス・ポリシーのセットアップ 

EU システム・アクセス・ポリシーを管理するには、**「管理」&gt;「EU システム・アクセス」&gt;「ポリシー」**をクリックします。

**「ポリシー」**セクションで、EU 外部の IBM 担当者がお客様のシステムにアクセスする場合に必要な承認レベルを指定できます。EU 以外の担当者による EU クラウド・システムへのアクセスには、IBM による事前の承認が必要です。選択可能な 3 つのポリシー・オプションによって、IBM の承認を含め、お客様の承認を必要とするかどうか、どのような場合に必要とするかが決まります。

<ul>
<li>お客様の承認は不要。IBM が承認した EU 外部の担当者は、お客様の承認なしでクラウド・システムにアクセスできます。</li>
<li>常にお客様の承認が必要。EU 外部の IBM 担当者は、明示的な承認がある場合にのみクラウド・システムにアクセスできます。</li>
<li>カスタム・ポリシー。アクセスが必要な保守アクティビティーのタイプと状態によってお客様の承認が必要になります。</li>
</ul>

ポリシーがお客様の承認を求めて呼び出しを行う場合、オプションで、アクセス要求が自動的に承認される_自動認証_ 期間を定義できます。

カスタム・ポリシーに関して、さまざまなタイプのアクティビティーおよび関連した状態を以下にリストし、説明します。

{: #ld_table18}

| **アクティビティー** | **状態** | **説明** |       
|-----------------|-------------------|-------------------|
| インシデント | ブロッカー | 指定されたコンポーネントは、アクセス不能であるか、予期したとおりに作動していません。環境に大きな影響を与えています。回避策はありません。|
| インシデント | 非ブロッカー | 指定されたコンポーネントに偶発的な問題が発生しているか、コンポーネントの一部分が予期したとおりに作動していません。環境に大きな影響はありません。または回避策が存在します。 |
| 保守更新 | 緊急デプロイメント| インシデントを解決または予防するために更新が必要です。|  
| 保守更新 | 通常デプロイメント| 更新は、サービスまたはコンポーネントの機能、使いやすさ、信頼性、またはパフォーマンスを強化するために行われる通常のアップグレードの一部です。 |
{: caption="表 18. EU カスタム・ポリシー" caption-side="top"}

### 未処理要求への対応 

ユーザーの処置を必要とするアクセス要求を表示するには、**「管理」&gt;「EU システム・アクセス」&gt;「未処理要求」**をクリックします。

**「未処理要求」**表内で、EU 外部の IBM 担当者からの EU システムへのアクセス要求を承認または拒否できます。未処理要求に関する一部の情報は表の列として表示されます。特定の要求の表の行を展開すると、追加の詳細が表示されます。  

1 つ以上の要求を承認または拒否するには、要求の左側にある 1 つ以上のチェック・ボックスを選択し、**「承認」**ボタンまたは**「拒否」**ボタンを選択します。アクセス要求が承認または拒否されると (これはお客様が手動で行うか、自動認証ポリシーを使って自動的に行うことができます)、そのレコードは**「未処理要求」**タブから**「要求履歴」**タブに移されます。アクセス要求を承認すると、システムにアクセスしているいろいろな IBM 担当者のレコードを表示できるようになります。 

**注**: アクセス要求は、特定の保守アクティビティーに関連付けられます。その保守アクティビティーは、複数の IBM 担当者によって実行される場合があります。あるアクセス要求を承認することにより、複数の IBM 担当者によるシステムへのアクセスを潜在的に承認することになります。クラウド・システムにアクセスする個人には、依然として IBM による個別の承認と資格情報が必要ですが、お客様は、更新またはインシデントに関連して必要なすべてのアクティビティーに対して 1 回認可するのみで済みます。 

各承認要求のサマリー情報が表の行に提供されます。表の行を展開すると追加情報が表示されます。

<dl>
<dt>状況</dt>
<dd>アクセス要求の状況。「保留中」、「承認 (Approved)」、または「拒否 (Rejected)」。</dd>
<dt>アクティビティー・タイプ</dt>
<dd>アクセスが必要なアクティビティーのタイプ。保守更新またはインシデント。</dd>
<dt>レコード ID</dt>
<dd>アクセスが必要なアクティビティーを追跡するために使用されるレコードの ID。アクティビティーが保守更新の場合、この値は実際のレコードにリンクされます。</dd>
<dt>要求日</dt>
<dd>お客様からアクセスの承認が要求された日時。</dd>
<dt>自動承認する時間 (Time to Auto-approval)</dt>
<dd>要求が自動的に承認される残り時間。アクセス・ポリシーの一部として自動承認が指定された場合にのみ適用されます。自動承認期間を設定した場合、割り当てられた担当者が IBM によって承認された時点で自動承認のカウントダウンが始まります。アクセス・ポリシーの一部として自動承認が指定された場合であっても、保守更新の承認要求では「自動承認する時間 (Time to Auto-approval)」が設定されないことがあります。これが事前承認要求である場合、詳細には、将来の特定の時刻を対象とすることを示すメモが含まれます。</dd>
<dt>説明</dt>
<dd>クラウド・システムへのアクセスを要求する理由付け。</dd>
<dt>重大度</dt>
<dd>インシデントの重大度を示します。インシデントの場合にのみ表示されます。</dd>
<dt>緊急デプロイメント</dt>
<dd>保守更新が緊急デプロイメントを必要とするかどうかを示します。保守更新の場合にのみ表示されます。</dd>
<dt>注記</dt>
<dd>将来デプロイされるようにスケジュールされている保守更新の一部として、要求が事前承認を必要としている場合、それを示します。そのような要求を承認すると、デプロイメントのための保守更新がスケジュールされている場合のクラウド・システムへのアクセスを事前承認することになります。要求を事前承認すると、それ以降のシステムが実際にアクセスされるときの承認は不要です。</dd>
</dl>

**注**: 通知サブスクリプションを構成して、新規アクセス要求が作成されたときに E メールを送信するか Web フックを起動するようにできます。詳しくは、**通知および通知サブスクリプション** (/docs/hybrid/index.html#oc_eventsubscription) を参照してください。

### 要求履歴の表示 

システムに対する以前のアクセス要求を表示するには、**「管理」&gt;「EU システム・アクセス」&gt;「要求履歴」**をクリックします。

**「要求履歴」**セクションでは、EU 外部の IBM 担当者が行った以前のアクセス要求をすべて表示できます。

各要求履歴のサマリー情報が表の行に示されます。表の行を展開すると追加情報が表示されます。

<dl>
<dt>状況</dt>
<dd>アクセス要求の状況。「承認 (Approved)」または「拒否 (Rejected)」。</dd>
<dt>アクティビティー・タイプ</dt>
<dd>アクセスが必要なアクティビティーのタイプ。保守更新またはインシデント。</dd>
<dt>レコード ID</dt>
<dd>更新またはインシデントのアクティビティーを追跡するために使用されるレコードの ID。アクティビティーが保守更新の場合、この値は実際のレコードにリンクされます。</dd>
<dt>要求日</dt>
<dd>お客様からアクセスの承認が要求された日時。</dd>
<dt>アクションの実行日</dt>
<dd>お客様によって要求が承認または拒否された日時。EU アクセス・ポリシーが IBM による承認のみを必要とするように構成されている場合、このフィールドには__「自動承認」__が表示されます。</dd>
<dt>説明</dt>
<dd>クラウド・システムへのアクセスを要求する理由。</dd>
<dt>重大度</dt>
<dd>インシデントの重大度を示します。インシデントの場合にのみ表示されます。</dd>
<dt>緊急デプロイメント</dt>
<dd>保守更新が緊急デプロイメントを必要とするかどうかを示します。保守更新の場合にのみ表示されます。</dd>
<dt>注記</dt>
<dd>要求に対して関連する追加の条件がある場合に記載されます。例えば、将来デプロイされるようにスケジュールされている保守更新の一部として、要求が事前承認を必要としている場合、このフィールドを使用してそれが示されます。</dd>
<dt>アクセス履歴</dt>
<dd>システムにアクセスしたあらゆる個人について、IBM によってアクセスが承認された日時、その個人のアクセスが認可および取り消された日時が表示されます。個人は、例えば、「IBM 担当者 1」などのように識別されます。</dd>
</dl>

**注**: アクセス・ポリシーが IBM による承認のみを必要とするように構成されている場合、新規アクセス要求は、自動的に**「承認 (Approved)」**状態に遷移し、**「要求履歴」**セクションに表示されるようになります。
